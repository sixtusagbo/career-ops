...
## Status
Round-1 corrections + fixups built, committed (not pushed), redeployed, and the "corrections are live" message sent. She approved M2 (Jun 20) and M2 is now active and funded. Round-2 inputs documented above. Round-2 build DONE 2026-06-22 (badges in the trust row, fixed to 64x64 square; EmailCapture extracted for de-dup; CronPlane waitlist live on /products; 33 tests green; commits 277ab49, aa9275d, 9929384, d46b3f4 plus 753ddd3 badge images). Badges reply, waitlist reply, and resource-files reminder all sent; the post-build closing is held until the round-2 corrections below land. Round-2 corrections requested on her localhost review (2026-06-22), prompt staged at docs/round2-corrections-prompt.md (not yet run): (1) certifications section bigger badges + more spacing, keep it a section not the footer (my call); (2) tech-strip logos reveal real brand colors on hover; (3) drop "tester" from the products waitlist line; (4) move the /api/subscribe go-live note from the internal PREVIEW-DEPLOY.md to the real handover doc deploy/DEPLOYMENT.md. BUILD-PLAN.md already untracked. M2 submit on hold (not rushing the money). Round-2 corrections applied and committed (5c7b5bd certs, aec0b49 tech hover, d569760 waitlist copy, 58af4ce deploy-guide note). Closing reply SENT 2026-06-22, then both repos pushed. Follow-up correction from her (2026-06-22, after the closing): the "Certifications" eyebrow must be left-aligned to the same margin as the WHY CRONEDGE / FAQ eyebrows (it was centered), and she wants a clear visual distinction between the certifications band and the FAQ (both were white with no boundary). Sixtus acked ("Yeah, for consistency"). APPLIED + committed 2026-06-22 (a972ce6 left-align eyebrow+badge row to the x=176 margin matching WHY CRONEDGE/FAQ; 03b3d5f certs band tone=cream + a border-t border-line hairline on Faq, since cream alone was only ~15 points off white). Sixtus: LGTM, pushed. Resource files: she started sending Wed Jun 24. File #1 received = "Small Business Cloud Security Checklist" (real CronEdge-branded PDF, ~212kB, staged untracked at the cronedge-website repo root). She sent the file only; drafted card copy: format "PDF checklist", desc "Find the security gaps that expose a small business to outages, data loss, and unauthorized access, across identity and access, infrastructure, and data protection." More coming. Plan: batch-wire the full real set once she's sent them all (replace both placeholders, move PDFs into public/resources/). Her files define the actual list, not a 1:1 swap.
